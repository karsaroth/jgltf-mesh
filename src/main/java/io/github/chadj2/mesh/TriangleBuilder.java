...
import io.github.chadj2.mesh.buffer.*;
...
public class TriangleBuilder extends TopologyBuilder {
  ...
  public TriangleBuilder(String _name, boolean shortIndices) {
  ...
  @Override public MeshVertex newVertex(Tuple3f _vertex) throws Exception {
    if(shortIndices && this._vertexList.size() >= ShortTriangleIndices.MAX_INDEX) {
      String msg = String.format("Trangle idex cannot exceed %d", ShortTriangleIndices.MAX_INDEX);
      throw new Exception(msg);
    }
    return super.newVertex(_vertex);
  }
  ...
  @Override protected void buildBuffers(MeshGltfWriter _geoWriter, MeshPrimitive _meshPrimitive) throws Exception {
    super.buildBuffers(_geoWriter, _meshPrimitive);

    if (this._material != null) {
      int _materialIdx = _geoWriter
          .getGltf()
          .getMaterials()
          .indexOf(this._material);
    ...
    TriangleIndices<?> indices = (shortIndices) ? new ShortTriangleIndices(this.getName()) : new UIntTriangleIndices(
        this.getName());
   ...
   */
  @Deprecated public Node build(MeshGltfWriter _geoWriter, Material _material) throws Exception {
